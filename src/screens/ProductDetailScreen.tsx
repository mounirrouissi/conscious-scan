...
export const ProductDetailScreen: React.FC<ProductDetailScreenProps> = ({
  ...
  const handleShare = async () => {
    try {
      const message = `Check out ${product.name} by ${product.brand}!\n\nScore: ${product.overallScore}/100 (${product.letterGrade})\n\n${product.personalizedWarnings.length > 0 ? 'Warnings:\n' + product.personalizedWarnings.join('\n') : 'No warnings!'}\n\nAnalyzed with ConsciousScan`;
        ...
        <html>
          ...
          <body>
            ...
            ${product.personalizedWarnings.length > 0 ? `
              <h2>⚠️ Warnings</h2>
              ${product.personalizedWarnings.map(w => `<div class="warning">${w}</div>`).join('')}
            ` : ''}
            
            ${product.personalizedAdvice.length > 0 ? `
              <h2>✓ Advice</h2>
              ${product.personalizedAdvice.map(a => `<p>${a}</p>`).join('')}
            ` : ''}
            ...
            <p style="margin-top: 40px; color: #9CA3AF; font-size: 12px;">
              Generated by ConsciousScan on ${new Date().toLocaleDateString()}
            </p>
          </body>
        </html>
      `;
          ...
          {concernIngredients.length > 0 && (
            <View style={styles.ingredientGroup}>
              <Text style={styles.groupTitle}>
                ⚠️ Ingredients of Concern ({concernIngredients.length})
              </Text>
